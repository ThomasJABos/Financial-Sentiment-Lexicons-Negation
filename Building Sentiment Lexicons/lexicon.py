...
Created on Sun Mar 22 14:38:32 2020
@author: ThomasBos
"""
import codecs as cd
import numpy as np
...
def save(folder, params, epoch):
    for param in params:
        if os.path.exists(os.path.join(folder, epoch + param.name + '_current.npy')):
            subprocess.call(['mv', os.path.join(folder, epoch + param.name + '_current.npy'),
                             os.path.join(folder, epoch + param.name + '_last.npy')])
...
def train_sowe(trainsentences,
    ...
    while (epoch < n_epochs):
        ...
        for minibatch in grouper:
            ...
            epoch, total_examples * 100. / num_trains), 'completed in %.2f (sec) <<\r' % (time.time() - tic),
            sys.stdout.flush()
            if (total_examples - last_examples) >= batch_size * report:
                ...
                t = time.time() - tic
                print
                "[Reporting] epoch ", epoch, ": training examples=", total_examples, ", speed=", total_examples / t, ' ' * 20
                print
                "==========> Average loss=", avg_cost, ", perfomance=", avg_perf, ' ' * 20
                save(folder, params, 'epoch_' + str(epoch))
        print
        '*' * 70
        print
        "\t\tFINISH TRAINING epoch ", epoch, 'in ', time.time() - tic
        print
        '*' * 70
...
if __name__ == "__main__":
    traininfofile = 'C:/Users/tjabo/PycharmProjects/Thesis/info.tw'
    trainfile = 'C:/Users/tjabo/PycharmProjects/Thesis/process.tw'
